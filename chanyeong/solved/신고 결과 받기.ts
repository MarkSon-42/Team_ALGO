(() => {
  const solution = (id_list: string[], report: string[], k: number) => {
    ...
    const reportedInfo: { [key in string]: { reportedCount: number } } = {};
    const reportInfo: { [key in string]: Set<string> } = {};
    for (const id of id_list) {
      reportedInfo[id] = { reportedCount: 0 };
      reportInfo[id] = new Set<string>();
    }
    for (const [reportUser, reportedUser] of reportUsers) {
      if (reportInfo[reportUser].has(reportedUser)) continue;
    ...
    for (const id of id_list) {
      ...
      for (const reportUser of reportInfo[id])
        if (reportedInfo[reportUser].reportedCount >= k) count++;
