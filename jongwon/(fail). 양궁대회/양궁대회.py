...
# 1. 라이언이 맞힌 과녁 점수의 조합을 만듭니다. 이때, 어피치가 맞힌 점수보다 큰 과녁 점수를 맞히면 라이언이 해당 점수를 가져갑니다. 
# 그렇지 않은 경우는 어피치가 해당 점수를 가져갑니다.
# 2. 라이언과 어피치의 점수를 계산하여 누가 이길 수 있는지 판단합니다.
# 3. 가능한 모든 조합을 계산하여 가장 큰 점수 차이로 이길 수 있는 조합을 찾습니다.
# 4. 점수차가 같으면 낮은 점수에 활을 더 많이 쏜 조합 선택
# 5. 라이언의 매 화살을 쏠 때의 점수를 추가할지 다음 화살로 넘어갈지 분기에서 가지가 계속 파생되어서 재귀 및 dfs 선택
